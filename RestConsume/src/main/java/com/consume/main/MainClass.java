...
public class MainClass {
		...
		if (usersMap != null) {
			for (LinkedHashMap<String, Object> map : usersMap) {
				System.out.println("Customer : id=" + map.get("id")
						+ ", firstName =" + map.get("firstName") + ", lastName="
						+ map.get("lastName") + ", email=" + map.get("email")
						+ ", mobile=" + map.get("mobile"));
			}
			
			String response = restTemplate.getForObject(REST_ENDPOINT_URI + "/customers/", String.class);
    ...
    private static void getCustomer(Integer id){
        ...
        RestTemplate restTemplate = new RestTemplate();
        Customer customer = restTemplate.getForObject(REST_ENDPOINT_URI+"/customers/" + id, Customer.class);
    	
        if (customer != null) {
				ResponseEntity<String> entity = restTemplate.getForEntity(REST_ENDPOINT_URI+"/customers/" + id, String.class);
    ...
    /* POST CUSTOMER */
    private static void createCustomers(Customer cust) {
...
//        Customer resp = restTemplate.postForObject(REST_ENDPOINT_URI+"/customers/", customer, Customer.class);
        ResponseEntity<Customer> entity= restTemplate.postForEntity(REST_ENDPOINT_URI+"/customers/", customer, Customer.class);
    ...
    /* DELETE CUSTOMER */
    private static void deleteCustomer(Integer id) {
    ...
    /* PUT CUSTOMER */
    private static void updateUser(Customer cust, Integer id) { 
        System.out.println("Testing update User API----------");
        RestTemplate restTemplate = new RestTemplate();
        Customer customer  = new Customer(cust.getId(),cust.getFirstName(),cust.getLastName(),cust.getEmail(),cust.getMobile());
        restTemplate.put(REST_ENDPOINT_URI+"/customers/" + id, customer);
    }

	public static void main(String[] args) {
		Customer cust = new Customer();
		cust.setFirstName("Mesut");
		cust.setLastName("Ozil");
		cust.setEmail("mesut.ozil@gmail.com");
